...
/**
 * Función auxiliar para obtener el nombre amigable (name) basado en la ruta (to).
 * @param {string} path - La ruta activa (e.g., '/', '/contactos').
 * @returns {string} El nombre amigable (e.g., 'Dashboard', 'Contactos') o la ruta original si no se encuentra.
 */
...
export default function MainSection() {
  ...
  // useEffect para manejar la navegación con el botón de "atrás/adelante" del navegador
  React.useEffect(() => {
    const handlePopState = () => setActivePath(window.location.pathname);
    window.addEventListener("popstate", handlePopState);
    return () => window.removeEventListener("popstate", handlePopState);
  ...
  // Obtenemos el nombre amigable para mostrarlo en el contenido
    ...
    <>
      <div className="dashboard-container">
        ...
        <main className="main-content">
          <div className="content-box">
            <h1>Bienvenido al Dashboard del Equipo 49</h1>
            {/* MODIFICACIÓN: Usamos activeName en lugar de activePath */}
            <p>
